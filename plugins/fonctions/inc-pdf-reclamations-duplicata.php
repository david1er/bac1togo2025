...
if ($num_table > 0) {
    $texte = "DUPLICATA-RELEVES DE NOTES DES CANDIDATS ";
...
//$pdf->ezText(utf8_decode("\n<b>$titre</b>\n"),13,array('justification'=>'center'));
    ...
    $tab = array('nom', 'prenoms', 'sexe', 'num_table', 'serie', 'intitule', 'ddn', 'ldn', 'eps', 'nom_photo', 'pays', 'etablissement', 'jury', 'moyenne', 'delib1', 'delib2', 'coeff_total', 'note', 'matiere', 'id_matiere', 'coeff_mat', 'id_type_note', 'type_note', 'lv2', 'efa', 'efb');
...
//        if($iPage>0)    {$iPage=$pdf->newPage();}
        ...
        $pdf->ezText(utf8_decode("\n\n<u><b>DUPLICATA DE RELEVE DE NOTES</b></u>"), 13, array('justification' => 'center'));
 ...
 */
        ...
        $pdf->ezText(utf8_decode("\nMoyenne Globale (A): <b>$moyenne_generale/20</b>"), 12, array('justification' => 'center'));
        ...
        if (is_array($data[3])) {
            ...
            $pdf->ezText(utf8_decode("Moyenne (B): <b>$moy[3]/20</b>\n"), 12, array('justification' => 'center'));
        }
...
if (mysqli_num_rows($result_chefLieu) > 0) {
    ...
    echo "Aucun résultat trouvé.";
        ...
        if (($delib1 == 'Oral') && ($delib2 == 'Passable')) {
            $delib = 'Passable';
        }
...
//        $pdf->ezSetDy(-5);
        ...
        $pdf->ezText(utf8_decode("\nDECISION DU JURY : <b>$texteMention</b>"), 9, array('justification' => 'left'));
        if ($data[3]) {
            $pdf->ezSetY($height - 730);
        } else {
            $pdf->ezSetY($height - 600);
        }
        $pdf->ezText(utf8_decode("\nDélibération du $date_delib à $chefLieu\nPrésident(e) du Jury : <b>$president_jury</b>"), 10, array('justification' => 'left'));

        $pdf->ezNewPage();
        //  $pdf->ezText(utf8_decode("Signature du Président du Jury\n\n\n<b>$president_jury</b>"), 11, $options2);
        $pdf->ezText(utf8_decode("Duplicata établi le <b>" . date('d') . '/' . date('m') . '/' . date('Y') . "</b> à Lomé\nLe Directeur"), 10, array('justification' => 'center'));
        $pdf->ezColumnsStop();

        if ($data[3]) {
            $pdf->ezSetY($height - 800);
        } else {
...
$pdfG = "liste_jury" . $jury . '_serie' . $serie . '_table' . $num_table;
