...
if (isset($_GET['annee'])) {
    ...
    $sql = "SELECT ser.serie, delib1, count(*) as nbre
    ...
    $cols = array('ser' => 'SERIES', 'ins' => 'INSCRITS', 'pre' => 'PRESENTS', 'ad' => 'ADMIS', 'tx' => 'TAUX');
    ...
    foreach ($tNbre as $serie => $tDelib1) {
        ...
        $data[] = array('ser' => $serie, 'ins' => $inscrits, 'pre' => intval($presents), 'ad' => intval($admis), 'tx' => $taux);
    ...
    $data[] = array('ser' => 'TOTAL', 'ins' => $tot_inscrits, 'pre' => $tot_presents, 'ad' => $tot_admis, 'tx' => $tot_taux);
