...
if ($type == 0) {
    ...
    $cols = array('N°' => utf8_decode('N°'), 'nom' => utf8_decode('Nom'), 'prenoms' => utf8_decode('Prénoms'), 'sexe' => 'Sexe', 'ddn' => 'Date Naissance',
        'centre' => utf8_decode('Centre d\'écrit.'), 'serie' => utf8_decode('Série'), 'etablissement' => utf8_decode('Etablissement.'),
        'motif' => utf8_decode('Motif'));
    $order = " ORDER BY ser.serie, can.nom, can.prenoms";
    $titre1 = 'DOSSIERS EN INSTANCE DE REJET DE PERSONNES AGEES';
}

if (isset($_GET['id_inspection'])) {
    ...
    $sql = "SELECT can.id_candidat, can.nom, can.prenoms, ser.serie, sex.sexe,  can.num_table, "
        . " can.ddn, can.ldn, eta.etablissement as etablissementR,cen.etablissement as centreR, ser.id as id_serie,"
        . " can.telephone "
        . " FROM  bg_ref_sexe sex, bg_ref_serie ser, bg_ref_etablissement eta, bg_ref_etablissement cen,   "
        . " bg_candidats can,bg_ref_inspection ins "
        . " WHERE can.annee=$annee AND cen.id = can.centre AND can.serie=ser.id AND can.sexe=sex.id "
        . " AND   can.centre=cen.id  AND can.etablissement=eta.id AND ddn < '$age_limte_date' "
        . " AND eta.id_inspection=ins.id AND ins.id=$id_inspection"
        . $andWhere
        . $order;
    $result = bg_query($lien, $sql, __FILE__, __LINE__);
    $colonnes = array('id_candidat', 'nom', 'prenoms', 'ddn', 'serie', 'sexe', 'etablissementR', 'centreR', 'id_serie', 'telephone', 'num_table');
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $i++;
        $data[$i] = array('N°' => $i, 'nom' => utf8_decode($nom), 'prenoms' => utf8_decode($prenoms), 'sexe' => $sexe, 'ddn' => $ddn, 'centre' => utf8_decode($centreR), 'serie' => $serie, 'etablissement' => utf8_decode($etablissementR), 'motif' => utf8_decode('Candidats trop agé(e)s'));
    }
    ...
    $pdf->ezText(utf8_decode("<b>REGION: $region INSPECTION: $inspection \n</b></b>"), 13, array('justification' => 'center'));
...
$pdfG = "Liste_candidat_age_instance_rejet" . $id_inspection;
$pdf->ezInsertMode(1, 1, 'before');
$pdf->ezInsertMode(0);

$title = "Liste des candidats agés en instance de rejet $annee";
