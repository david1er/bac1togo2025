...
$pdf->ezStartPageNumbers(15, 15, 8, 'right', "Etablissement : $etab  ---  Centre d'ecrit : $centre");
...
if ($type == 0) {
    ...
    $cols = array('N°' => utf8_decode('N°ENR'), 'num_table' => utf8_decode('N°TABLE.'), 'noms' => 'CANDIDAT', 'dldn' => utf8_decode('DATE ET LIEU DE NAISSANCE'), 'sex' => 'Sexe', utf8_decode('pdn') => 'PAYS', utf8_decode('nat') => 'NAT.', 'serie' => 'SERIE', 'eps' => 'EPS', 'lv2' => 'LV2', 'efa' => 'EF. A', 'efb' => 'EF. B', 'sig' => 'EMARGEMENT');
...
if (isset($_GET['id_etablissement'])) {
    ...
    $sql = "SELECT can.id_candidat, can.nom, can.prenoms, ser.serie, sex.sexe, can.annee_bac1, can.jury_bac1, can.num_table_bac1, can.num_dossier, can.num_table,can.statu, atelier1, atelier2, atelier3, "
        ...
        . " AND can.statu=1 AND eta.id =$id_etablissement AND can.motif!=0 "
        . $andWhere
        . $order;
    $result = bg_query($lien, $sql, __FILE__, __LINE__);
    $colonnes = array('id_candidat', 'nom', 'prenoms', 'serie', 'sexe', 'annee_bac1', 'jury_bac1', 'num_table_bac1', 'ddn', 'ldn', 'pdn', 'nationalite', 'etablissement', 'id_serie', 'lv2', 'telephone', 'ville', 'eps', 'efa', 'efb', 'id_eps', 'num_dossier', 'num_table', 'atelier1', 'atelier2', 'atelier3', 'statu');
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $dldn = $ddn . utf8_decode(html_entity_decode(" à ")) . utf8_decode($ldn);
        ...
        $i++;
        $data[$i] = array('N°' => $id_candidat, 'num_table' => "xxx", 'noms' => utf8_decode($candidat), 'sex' => $sexe, 'dldn' => $dldn, 'pdn' => utf8_decode($pdn), 'nat' => utf8_decode($nationalite), 'serie' => $serie, 'eps' => $eps . ' ' . $atelier, 'lv2' => $lv2, 'efa' => utf8_decode($efa), 'efb' => utf8_decode($efb), 'sig' => '', 'annee1' => $annee_bac1, 'jury1' => $jury_bac1, 'table1' => $num_table_bac1);
    }
