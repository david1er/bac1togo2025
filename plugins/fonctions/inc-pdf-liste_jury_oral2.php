...
if ($jury > 0) {
    ...
    $sql = "SELECT can.nom, can.prenoms, can.sexe, can.num_table, ser.serie, cen.etablissement, rep.jury, delib1, delib2, mat.id as id_matiere, mat.matiere, mat.abreviation
		FROM bg_candidats can, bg_repartition rep, bg_ref_serie ser, bg_ref_pays pay, bg_ref_etablissement cen, bg_resultats res, bg_ref_matiere mat, bg_calendrier calend
		WHERE can.annee=$annee AND rep.annee=$annee AND res.annee=$annee
        AND can.num_table=rep.num_table AND rep.id_anonyme=res.id_anonyme
		 AND can.serie=ser.id AND can.pdn=pay.id
		AND cen.id=rep.id_centre AND rep.jury=$jury AND mat.id=calend.id_matiere AND delib1='Oral' AND calend.id_type_note=3
