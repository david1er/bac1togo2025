...
$pdf->ezStartPageNumbers(570, 15, 8, 'left', "jury $j / $DatePied - Page {PAGENUM} sur {TOTALPAGENUM}");

$options = array('titleFontSize' => 16, 'fontSize' => 14);
...
$sql = "SELECT can.num_table, ser.serie, rep.jury, rep.id_anonyme as num_anonyme
		...
		AND rep.jury=$j AND can.type_session=$s AND rep.id_type_session=$s
...
$cols = array('Num' => 'NUMERO DE TABLE', 'Ano' => 'ANONYMAT');
//$titre_tab='Jury '.$j;
...
$pdf->ezInsertMode(0);
$pdfG = "anonymat_jury" . getRewriteString($j);
