...
if (isset($_GET['id'])) {
    $sql = "SELECT *,ins.inspection ,ins.id  FROM bg_ref_etablissement eta , bg_ref_inspection ins WHERE eta.id_inspection=ins.id  AND eta.id=$id AND login_eta!='' AND mdp_eta!='' AND id_etat=1 ORDER BY etablissement ";
...
if (isset($d) && $d) {
    ...
    $pdf_fic = "pdf/etablissement_" . $annee . "_$id.pdf";
    $fp = fopen($pdf_fic, 'wb');
    fwrite($fp, $pdfcode);
    fclose($fp);
