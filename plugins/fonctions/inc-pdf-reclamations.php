...
//$pdf->ezText(utf8_decode("\n<b>$titre</b>\n"),13,array('justification'=>'center'));
    ...
    $tab = array('nom', 'prenoms', 'sexe', 'num_table', 'serie', 'intitule', 'eps', 'ddn', 'ldn', 'nom_photo', 'pays', 'etablissement', 'jury', 'moyenne', 'delib1', 'delib2', 'coeff_total', 'note', 'matiere', 'id_matiere', 'coeff_mat', 'id_type_note', 'type_note', 'lv2', 'efa', 'efb');
...
if (mysqli_num_rows($result_chefLieu) > 0) {
    ...
    echo "Aucun résultat trouvé.";
        ...
        if ($data[3]) {
            $pdf->ezSetY($height - 700);
        } else {
            $pdf->ezSetY($height - 580);
        }
        $pdf->ezNewPage();
