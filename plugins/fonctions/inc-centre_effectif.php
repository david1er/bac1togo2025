...
$sql = "SELECT reg.region, cent.etablissement as centre,cod.code, cod.jury as jur, count(*) as effectif
...
$data = array();
$colonnes = array('region', 'centre', 'jur', 'code', 'effectif');
while ($row = mysqli_fetch_array($result)) {
    foreach ($colonnes as $col) {
        $$col = utf8_encode($row[$col]);
    }

    $data[$jury][] = array('centre' => $centre, 'jur' => $jur, 'code' => $code, 'effectif' => $effectif);
}

$cols = array('centre' => 'CENTRE', 'jur' => 'JURY', 'code' => 'CODE', 'effectif' => 'EFFECTIF');
//$titre_tab = $centre;
