...
if ($jury > 0) {
    ...
    foreach ($tNum as $num_table) {
            ...
            if (in_array($id_type_note, $tTypes[$num_table])) {
                ...
                foreach ($tMatieres[$num_table][$id_type_note] as $id_matiere => $matiere) {
                    if ($id_type_note == 4) {
                        ...
                        if ($id_matiere == 51) {
                            $matiere = selectReferentiel($lien, 'langue_vivante', $lv2);
                            $data[$id_type_note][$i]['matiere'] = ('Langue Vivante 2 (' . $matiere . ')');
                        } else {
                            $data[$id_type_note][$i]['matiere'] = ($matiere);
                        }
        ...
        $sql_resultats = "SELECT * FROM bg_resultats WHERE num_table='$num_table' LIMIT 1";
        $result_resultats = bg_query($lien, $sql_resultats, __FILE__, __LINE__);
        $sol_resultats = mysqli_fetch_array($result_resultats);
        //$total_points = $sol_resultats['total'];
                ...
                if ($id_type_note != 3) {
                    ...
                    $dataTotal[$id_type_note][$i]['points_mat'] = $sol_resultats['total']; //$total_points + $tabPoints[4];
                 ...
                 */
                if ($id_type_note == 1) {
                    $total_points_ecrits = $total_points;
        ...
        $pdf->ezText(utf8_decode("Moyenne Globale (A): <b>$moyenne_generale/20</b>"), 12, array('justification' => 'center'));
        $pdf->ezSetDy(-10);

        if (is_array($data[3])) {
            $titre_tableau = "EPREUVES ORALES DE CONTROLE (B)";
            $pdf->ezTable($data[3], $entete, $titre_tableau, $options);
            $moyenne_2 = str_replace('.', ',', substr($moy[3], 0, 6));
            $pdf->ezText(utf8_decode("Moyenne (B): <b>$moyenne_2/20</b>"), 12, array('justification' => 'center'));
        }
        ...
        if ($jury >= 1 and $jury < 299) {
            $chefLieu = "Lomé";
        } else if ($jury >= 300 and $jury < 499) {
            $chefLieu = "Tsevié";
        } else if ($jury >= 500 and $jury < 599) {
            $chefLieu = "Atakpamé";
        } else if ($jury >= 600 and $jury < 699) {
            $chefLieu = "Kpalimé";
        } else if ($jury >= 700 and $jury < 799) {
            $chefLieu = "Sokodé";
        } else if ($jury >= 800 and $jury < 899) {
            $chefLieu = "Kara";
        } else if ($jury >= 900 and $jury < 999) {
            $chefLieu = "Dapaong";
        } else if ($jury >= 1001 and $jury < 3000) {
            $chefLieu = "Lomé";
        }
        ...
        if (in_array($delib, array('Passable', 'Abien', 'Bien', 'TBien'))) {
            $texteMention = strtoupper("Admis$e avec la mention \n$mention");
        } else {
