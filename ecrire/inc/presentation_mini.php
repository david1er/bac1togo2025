...
/**
 * Retourne une information lié au chronogramme pour l'espace privé
 *
 * Préférez l'usage en squelettes via la balise `#INFOS_CHRONOGRAMME`.
 *
 * @see infos_chronogramme_ecrire_charger_dist()
 *
 * @param string $page Nom de la page exec
 * @param string $complement Code HTML supplémentaire
 * @return string             Code HTML
 **/
function infos_chronogramme($page, $complement = "",$lien) {
	// $recherche = _request('recherche');
	// $recherche_aff = entites_html($recherche);
	// if (!strlen($recherche)) {
	// 	$recherche_aff = _T('info_rechercher');
	...
	//#FORMULAIRE_RECHERCHE_ECRIREfunction afficherEtapeExamen($lien)
